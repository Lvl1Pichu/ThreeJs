...
function createFireplaceEnvironment() {
  ...
  const loader = new FBXLoader();
    ...
    const texture = textureLoader.load("./textures/HouseTextures.png"); // Adjust the path if necessary

    loader.load(
    "./source/untitled.fbx", // Path to your FBX file
    (fbx) => {
        fbx.scale.set(0.005, 0.005, 0.005); // Further reduce the scale
        fbx.position.set(100, -1, 3); // Position the model near the fireplace
        fbx.rotation.y = Math.PI / 2; // Rotate the model to face the fireplace
        fbx.traverse((child) => {
        if (child.isMesh) {
            child.castShadow = true;
            child.receiveShadow = true;

            // Apply the loaded texture if no material exists
